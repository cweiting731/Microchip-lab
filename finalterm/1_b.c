...
void Mode2() {   // Todo : Mode2 
    Servo_WriteAngle(m2Angle + 90, 16);
    return ;
}
...
void Mode4() {
    ...
    if (servoInit == 0) {
        ...
        m1Angle = (uint32_t)adcValue * 180 / 1023;
        // Servo_WriteAngle(angle, 16);
        char buf[20];
        sprintf(buf, "%4d", m1Angle - 90);
        UART_Write_Text(buf);
    } else {
...
void commandHandler() {
    ...
    if (currentMode == 2) {
        ...
        if (endptr != str && *endptr == '\0') {
            ...
            char buf[30];
            m2Angle = (int)val;
            if (m2Angle < -90 || m2Angle > 90) {
                UART_Write_Text("Error: Angle out of range (-90 to 90)\r\n");
                currentMode = 0;
                promptPrinted = false;
                ClearBuffer();
                return;
            }
            sprintf(buf, "Switch angle: %d\r\n", m2Angle);
            UART_Write_Text(buf);
            currentMode = 0;
            promptPrinted = false;
            Servo_WriteAngle(m2Angle + 90, 16);
    ...
    else if(strncmp(str, "m2", 2) == 0){
        // Timer0_Initialize(64285, 8, 0); // 初始化 Timer0

        currentMode = 2;
        UART_Write_Text("===============\r\n");
        UART_Write_Text("m2 Enter\r\n");
        // // 顯示現在delay時間
        // char buf[30];
        // sprintf(buf, "Switch angle: %d\r\n", m2Angle);
        // UART_Write_Text(buf);
    ...
    else if (strncmp(str, "m1", 2) == 0) {
        // ADC_Initialize(0b1110, 0, 0, 0b010, 0b100, 1); // AN0 analog, Vref=Vdd/Vss, 4Tad, Fosc/4, right justified
         // 50Hz for servo
        currentMode = 4;
        UART_Write_Text("===============\r\n");
        UART_Write_Text("m1 Enter\r\n");
    }
...
void main(void) 
{
    ...
    while(1) {
        if (promptPrinted == false) {
            UART_Write_Text("Select Mode (m1/m2): ");
            promptPrinted = true;
        }
