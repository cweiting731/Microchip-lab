...
void Mode3() {   // Todo : Mode3 
    if (done) return ;
    uint16_t transferValue = ADC_Read(0);
    uint16_t transferAngle = (uint32_t)transferValue * 180 / 1023;
    Servo_WriteAngle(transferAngle, 16);
    // char buf[5];
    // UART_Write('\b');
    // UART_Write('\b');
    // UART_Write('\b');
    // UART_Write('\b');
    // sprintf(buf, "%4u", transferValue);
    // UART_Write_Text(buf);
    ...
    if (PORTBbits.RB0 == 0) {
        ...
        if (state == 1) {
            x1 = transferAngle - 90;
            char buf[30];
            sprintf(buf, "x1 set to %d\r\n", x1);
            UART_Write_Text(buf);
        } else if (state == 2) {
            x2 = transferAngle - 90;
            char buf[30];
            sprintf(buf, "x2 set to %d\r\n", x2);
            UART_Write_Text(buf);
        } else {
            bool isIncreasing = true;
            uint16_t angle = 0;
            Servo_WriteAngle(0, 16); // reset servo
            __delay_ms(200);
            char buf[30];
            sprintf(buf, "start transfer %d\r\n", x1 + x2);
            UART_Write_Text(buf);
            int total = x1 + x2;
    ...
    if (hardInit == 0) {
        updateLATDdigit(transferValue / 64);
        hardInit = 1;
    } else {
...
unsigned char ADCmap() {
    ...
    Servo_WriteAngle((uint32_t)adcValue * 180 / 1023, 16);

    

    if (adcValue < 64) return 0;
    else if (adcValue < 128) return 1;
    else if (adcValue < 192) return 2;
    else if (adcValue < 256) return 3;
    else if (adcValue < 320) return 4;
    else if (adcValue < 384) return 5;
    else if (adcValue < 448) return 6;
    else if (adcValue < 512) return 7;
    else if (adcValue < 576) return 8;
    else if (adcValue < 640) return 9;
    else if (adcValue < 704) return 10;
    else if (adcValue < 768) return 11;
    else if (adcValue < 832) return 12;
    else if (adcValue < 896) return 13;
    else if (adcValue < 960) return 14;
    else return 15;
}